...
a0 = 0.; ##background
A = 0.3; G = 5;
a = 0.3; dm = 0.01; dc = 0.001; kon = 0.0; koff = 0.0; em = 0.; ew = 10;
g0 = 0.0; g = 5; dw = 0.01; w0 = 430; m0 = 25; m1 = 1; m2 = 0.5;
kw = 0.0015; km = 0.03; n = 2; m = 2; q = 2;
...
def equations_wt(X):
    fwt = [a0 + a*((1 + em*(km*X[0])**n )/(1 + (km*X[0])**n ))*((1 + ew*(kw*X[1])**n )/(1 + (kw*X[1])**n )) - kon*X[0]*X[1] + koff*X[2] - dm*X[0]]
    fwt.append(g0 + (g*(w0**m)/(w0**m + (X[1])**m))*(m1 + m2*(X[0]**q)/(m0**q + X[0]**q) ) - kon*X[0]*X[1] + koff*X[2] - dw*X[1])
    fwt.append(kon*X[0]*X[1] - koff*X[2]  - dc*X[2])
    return fwt

def equations_bm(X):
    fbm = [X[0]]
    fbm.append(g0 + m1*g*(w0**m)/(w0**m + (X[1])**m) - dw*X[1])
    fbm.append(X[2])
    return fbm

def equations_wm(X):
    fwm = [a0 + a*((1 + em*(km*X[0])**2)/(1 + (km*X[0])**2)) - dm*X[0]]
    fwm.append(X[1])
    fwm.append(X[2])
    return fwm
...
for jj in KK:
    ...
    pw = g0 + (gg[:,j]*(w0**m)/(w0**m + (Zwt[:,j])**m))*(m1 + m2*(Xwt[:,j]**q)/(m0**q + Xwt[:,j]**q) )

    sm = (a0 + aa[:,j]*(1 + ew*(kw*Zbm[:,j])**n)/(1 + (kw*Zbm[:,j])**n))/pm
    sw = (g0 + (gg[:,j]*(w0**m)/w0**m))*(m1 + m2*(Xwm[:,j]**q)/(m0**q + Xwm[:,j]**q) )/pw
    
    smd = (a0 + aa[:,j]*(1 + em*(km*Xwm[:,j])**n)/(1 + (km*Xwm[:,j])**n))/pm
    swd = (g0 + m1*gg[:,j]*(w0**m)/(w0**m + (Zbm[:,j])**m))/pw
    
    smdd = (a0 + aa[:,j])/pm
    swdd = (g0 + m1*gg[:,j]*(w0**m)/w0**m)/pw
...
plt.savefig('bounds_bravo.pdf', transparent = True, bbox_inches='tight')
